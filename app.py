...
class App(ctk.CTk):
    ...
    def _do_lock(self):
        ...
        if result["success"]:
            ...
            try: self.lpw.reset() except: pass
            try: self.lbw.reset() except: pass
            self._lock_pattern=""; self._lock_both_pattern=""
            try: self.lpl.configure(text="Draw pattern (min 4 dots)",text_color=TEXT_MUTED) except: pass
            try: self.lbl2.configure(text="Also draw pattern",text_color=TEXT_MUTED) except: pass
